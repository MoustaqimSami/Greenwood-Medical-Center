(function () {
  ...
  const { appointments, updateAppointmentById } = window.appointmentsDatabase;
  ...
  const firstApptDate = appointments[0] ? new Date(appointments[0].date) : new Date();
  ...
  const completedPanel = document.getElementById("completed-appointments-panel");
  ...
  quickActions.forEach((qa) => {
    qa.addEventListener("click", () => {
      ...
      const label = labelElement ? labelElement.textContent.trim() : "Quick action";
  ...
  function renderAppointmentCard(appt, statusView) {
    ...
    if (statusView === "upcoming") {
      ...
      completeBtn.className = "btn btn--small btn--txt appointment-complete-btn";
      ...
      cancelBtn.className = "btn btn--small btn--danger-txt appointment-cancel-btn";
      cancelBtn.innerHTML = `
        <svg xmlns="http://www.w3.org/2000/svg" class="btn-icon btn-icon--small">
      ...
      followUpBtn.className = "btn btn--small btn--sec-txt appointment-followup-btn";
  ...
  completedPanel.addEventListener("click", (e) => {
